...
\begin{st}
  ...
  \begin{proof}
    ...
    \begin{itemize}
    \item $Z(R),+$ is een commutatieve deelgroep van $R,+$.\eiref{ei:centrum-is-deelgroep}
...
\begin{ei}
  \eiref{ei:factorisatie-breukenveld}
...
\begin{st}
  ...
  \begin{proof}
    ...
    Daaruit volgt meteen dat $I$ gelijk is aan $R$.\stref{st:eenheidselement-in-ideaal-maakt-hele-ring}
  \end{proof}
\end{st}
...
\begin{st}
  ...
  \begin{proof}
    ...
    \item $\Rightarrow$\\
      ...
      $I$ moet dan heel $R$ zijn.\stref{st:eenheidselement-in-ideaal-maakt-hele-ring}
    \item $\Leftarrow$\\
      Uit het ongerijmde:
...
\begin{de}
  ...
  Het \term{ideaal voortgebracht door een element} $a$, \term{principaal ideaal} of \term{hoofdideaal} door $a$ is $(a)$.
...
\begin{st}
  ...
  Zij $R,+,\cdot$ een ring en $S,+,\cdot$ een deelring van $R,+,\cdot$.
  Zij bovendien $I$ een ideaal van $R,+,\cdot$
  \[ I \triangleleft S+I \]
  \[ S \cap I \triangleleft S \]
  \[ \nicefrac{S}{S\cup I} = \nicefrac{S+I}{I} \]
  \begin{proof}
    \begin{itemize}
    \item $S+I$ is alvast een deelgroep van $R,+$\stref{st:groep-commutatief-HK-is-H-unie-K}.
      Omdat $I$ een ideaal is van $R,+,\cdot$ is $S+I$ bovendien een deelring van $R,+,\cdot$ en $I$ ook een ideaal van $S+I$.
    \item
    \item
    \end{itemize}
  \end{proof}
...
\begin{ei}
  \label{ei:maximaal-dan-priemideaal}
...
\begin{st}
  \examen
  \stref{st:quotientgroepen-asa-idealen-stelling}
...
\begin{st}
  \label{st:priemideaal-gegenereert-door-priem-in-hid}
  Zij $P=(p)$ een priemideaal van een HID $R,+,\cdot$ met eenheidselement $i$ en nulelement $e$, dan is $p$ een irreducibel element in $R,+,\cdot$.

  \begin{proof}
    Zij $P$ een priemideaal gegenereert door een element $p$.
    Zij $a$ en $b$ twee elementen in $R,+,\cdot$ zodat $a\cdot b = p$ geldt.
    Dan moet \'e\'en van de twee in $P$ zitten omdat $P$ een priemideaal is.
    Noem dat element nu $b$.
    Er bestaat dan een element $r\in R$ zodat $rp=a$ geldt.
    \[
    \begin{array}{rrll}
      & rp &= b\\
      \Rightarrow & rab &= b & (p=ab) \\
      \Rightarrow & rab - b &= e\\
      \Rightarrow & (ra-1)b &= e\\
      \Rightarrow & (ra-1) &= e & (R \text{ heeft geen nuldelers en } b\neq e)\\ 
      \Rightarrow & ra &= i\\
    \end{array}
    \]
    Dit houdt in dat $a$ een eenheid is.
    $p$ is dus irreducibel.
  \item $\Leftarrow$\\
    Zij $p$ een irreducibel element in $R,+,\cdot$.
    Beschouw nu het ideaal $P=(p)$, voortgebracht door $p$.
    Kies een willekeurig element $x\in P$ en stel dat het geschreven kan worden als $ab=x$.
    We moeten nu bewijzen dat ofwel $a$ in $P$ zit, ofwel $b$.
  \end{proof}
\end{st}
...
\begin{gev}
  ...
  \begin{proof}
    $R[X],+,\cdot$ is commutatief\stref{st:veeltermen-comm-asa-ring-comm}, heeft een eenheidselement\stref{st:veeltermen-eenh-asa-ring-eenh} en geen nuldelers\stref{st:veeltermen-geen-nuldelers-asa-ring-geen-nuldelers} als hetzelfde geldt voor $R,+,\cdot$.
  \end{proof}
\end{gev}

\begin{st}
  \label{st:integriteitsdomein-veeltermring-eenheden}
  \examen
  Zij $R,+,\cdot$ een integriteitsdomein, dan geldt het volgende:
  \[ (R[X])^{\times} = R^{\times} \]
 
  \begin{proof}
    $R,+,\cdot$ heeft geen nuldelers.
    Zij $x$ een eenheid in $R[X]$, dan bestaat er een inverse $y$ zodat het volgende geldt (waarbij $i$ het eenheidselement is van $R,+,\cdot$ en dus ook van $R[X],+,\cdot$\stref{st:veeltermen-eenh-asa-ring-eenh}).
    \[ x \cdot y = i\]
    Vanwege de regel voor de graad van het product van veeltermen\stref{st:veeltermen-geen-nuldelers-asa-ring-geen-nuldelers} moeten zowel $x$ als $y$ graad $0$ hebben.
    De enige elementen met een invers voor $\cdot$ zijn dus de constante veeltermen die een inverse hebben.
  \end{proof}
\end{st}
...
\begin{st}
  ...
  \begin{proof}
    Als de ring $R,+,\cdot$ (en dus ook $R[X],+,\cdot$) een integriteitsdomein is, dan is $R[X],+,\cdot,gr$ een euclidische ring en kunnen we het algoritme van Euler gebruiken.\alref{al:algoritme-van-euclides}
...
\begin{st}
  ...
  \begin{proof}
    ...
    \begin{itemize}
    \item Het delingsalgoritme\stref{st:delingsalgoritme} van $f$ door $(X-a)$ levert een $q$ en $r$ op zodat het volgende geldt.
...
\begin{st}
  ...
  \begin{proof}
    ...
    \begin{itemize}
    \item De bewering geldt voor $n=1$.\stref{st:reststelling}
    ...
    \item ... en bewijzen dat daaruit volgt dat de bewering geldt voor $n=k+1$.
      ...
      $a_{r}$ is een nulpunt van $g$ en dus is $X-a_{k+1}$ een deler van $g$.\stref{st:reststelling}
...
\begin{ei}
  ...
  \begin{proof}
    ...
    \item $\Leftarrow$\\
      Als $f$ een nulpunt heeft, geldt $X-a \mid f$ \stref{st:reststelling} voor $a$ een nulpunt van $f$.
...
\begin{de}
  ...
  De \term{polynomenring} of \term{veeltermenring} over $R$ in de veranderlijken $X_{j}$, genoteerd $R[X_{j}]_{j\in V}$ is de verzameling van alle formele sommen van de volgende vorm, uitgerust met de veelterm-som en het veelterm-product:
...
\begin{de}
  ...
  Een element $x\in R$ is \term{irreduceerbaar} of \term{irreducibel} in $R,+,\cdot$ als het volgende geldt:
  \begin{itemize}
...
\begin{st}
  ...
  \begin{proof}
    We weten als dat $F[X]$ een integriteitsdomein is.\gevref{gev:veeltermen-domein-asa-ring-domein}
    ...
    Kies dus een element $g$ uit $I\setminus \{e_{F}\}$ van minimale graad, dan wordt $I$ voortgebracht door $g$.\lemref{lem:minimale-veelterm-brengt-ideaal-voort}
  \end{proof}
\end{st}
...
\begin{st}
  \label{st:z-is-hid}
  $\mathbb{Z},+,\cdot$ is een HID.

  \begin{proof}
    Zij $I,+,\cdot$ een ideaal van $\mathbb{Z},+,\cdot$, dan is $I,+$ cyclisch omdat het een deelgroep is van de cyclische groep $\mathbb{Z},+$.\stref{st:deelgroep-van-cyclische-groep-is-cyclisch}
    Er bestaat dus een generator $n$ voor $\mathbb{I}$:
    \[ I = \{ k \cdot n \mid n \in \mathbb{Z} \} = (n) \]
    $I$ is dus een hoofideaal.
  \end{proof}
\end{st}

\begin{st}
  Voor elke $n$ is $n\mathbb{Z}$ een ideaal van $\mathbb{Z}$.

  \begin{proof}
    Voor elke $n\in \mathbb{Z}$ beschouwen we de deelring $n\mathbb{Z}$.
    Beschouw nu een willekeurig element $m \in \mathbb{Z}$:
    \[ m\cdot n | n \Rightarrow m\cdot n \in n\mathbb{Z}\]
    Bovendien zijn zowel $n+n'$ als $-n$ elementen van $n\mathbb{Z}$ voor elke $n'\in\mathbb{Z}$.
    $n\mathbb{Z}$ is dus een ideaal van $\mathbb{Z},+,\cdot$.
  \end{proof}
\end{st}
...
\begin{lem}
  ...
  \begin{proof}
    ...
    \item Als $d$ een eenheid is, bestaan er elementen $u$ en $v$ in $R$ zodat het volgende geldt\stref{st:stelling-van-bezout}:
      \[ i = ua + vp \]
...
\begin{st}
  ...
  \begin{proof}
    ...
    \item Bestaan\\
      ...
      Stel dat $x_{0}$ niet te ontbinden valt in irreducibele elementen (dan is $x_{0}$ niet irreducibel want als $x_{0}$ irreducibel was dan was $x_{0}$ zelf al de ontbinding van $x_{0}$ in irreducibele elementen), dan bestaan er elementen $x_{1}$ en $y_{1}$ in $R$ zodat $x_{0}$ als volgt te schrijven valt.
      \[ x_{0} = x_{1}y_{1} \]
      Hierin zijn $x_{1}$ en $y_{1}$ geen eenheden\deref{de:irreducibel} en kan ofwel $x_{1}$ ofwel $y_{1}$ zelf ook niet ontbonden worden in irreducibele elementen.
      Als zowel $x_{1}$ als $y_{1}$ immers een product waren van irreducibele elementen, dan was $x_{0}$ dat ook.
      ...
      Nu is de hoofdideaal door $x_{0}$ een deel van de hoofdideaal door $x_{1}$, maar niet gelijk.
      \[ (x_{0}) \subsetneq (x_{1}) \]
      ...
      \[ (x_{0}) \subsetneq (x_{1}) \subsetneq (x_{2}) \subsetneq \dotsb \subsetneq (x_{n}) \subsetneq (x_{n+1}) \subsetneq \dotsb\]
      Dit is natuurlijk onmogelijk.\lemref{lem:hid-keten-van-idealen-stabiliseert}
    \item Uniciteit\\
      ...
      \item ... en bewijzen dan dat de bewering ook geldt voor $m=k+1$.\\
        ...
        Dit betekent ook het volgende\lemref{lem:irreducibel-element-in-HID-deelt}:
