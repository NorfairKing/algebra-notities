...
\begin{de}
  Een \term{halfgroep} of \term{mono\"ide} $G,*$ is een algebra\footnote{Zie definitie \ref{de:algebra}.} die bestaat uit een (niet-lege) verzameling $G$ en een afbeelding $*$ (De bewerking).
...
\begin{de}
  ...
  Een element $axa^{-1}$ met $a\in G$ in $G$ noemen we een \term{geconjugeerde} van $x$.
\end{de}
...
\TODO{bewijs pagina 19.}
\end{ei}

\begin{st}
  Zij $G,*$ een groep en $a$ en $b$ twee elementen van $G$.
  \[ Cl(a) = Cl(b) \Leftrightarrow b \in Cl(a) \Leftrightarrow Cl(b) \]
\end{st}
